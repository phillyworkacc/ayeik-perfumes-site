...
export async function dalRequireAdminRedirect() {
   const user = await getCurrentUser();
   if (user == null) redirect("/account/login");
   if (user.userType !== "admin") redirect("/account");
   return {
      userid: user.userid,
      name: user.name,
      email: user.email,
...
export async function dalRequireAdmin<T, E extends DalError>(
  operation: (user: typeof usersTable.$inferSelect) => Promise<DalReturn<T, E>>
) {
   const user = await getCurrentUser();
   if (user == null) return createErrorReturn({ type: "no-user" });
   if (user.userType !== "admin") return createErrorReturn({ type: "no-access" });
   return await operation(user);
}
