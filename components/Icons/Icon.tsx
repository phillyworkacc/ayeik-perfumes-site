...
import { FaFacebook, FaInstagram, FaTiktok, FaLinkedin, FaYoutube, FaXTwitter } from "react-icons/fa6";
...
export function Logo ({ size }: IconProps) {
   return (
      <div className='icon' style={{ width: `${size}px`, height: `${size}px` }}>
         <img src={websiteConfig.logo.src} alt="logo" />
      </div>
   )
}
...
export function SocialMediaIcon ({ socialMediaKey, size, color }: { socialMediaKey: SocialMediaPlatform, size: number, color?: string }) {
   const props = { size, color }
   const socialMediaIconMap: Record<SocialMediaPlatform, ReactElement> = { 
      "instagram": <FaInstagram {...props} />,
      "tiktok": <FaTiktok {...props} />,
      "facebook": <FaFacebook {...props} />,
      "x": <FaXTwitter {...props} />,
      "linkedIn": <FaLinkedin {...props} />,
      "youtube": <FaYoutube {...props} />,
   }

   return socialMediaIconMap[socialMediaKey];
}
